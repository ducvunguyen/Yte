...
<form action="{{route('admin.about.update', $data->id)}}" method="POST" role="form"  enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="file" name="image" id="imagePd" class="form-control" value="<?php echo $data->image ?>">
    </div>
    <div class="form-group">
        <label for="">Laravel </label>
        <input type="text" class="form-control" id="" placeholder="Nhập đường dẫn"  name="link" value="{{$data->links}}">
    </div>
    <div class="form-group">
